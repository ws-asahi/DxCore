...
USB support is now part of the core: USB CDC `Serial`, the standard Arduino Pluggable USB API (HID works out of the box; MIDI via MIDIUSB), and an optional USB CDC bootloader - select the "AVR DU-series (USB CDC Bootloader)" board to upload sketches over USB with no programmer needed.
