...
void usbInit(void) {
    ...
    /* 2. VUSB supply. On 5V systems the internal USB voltage regulator
     *    (USB VREG) generates the 3.3V VUSB rail from VDD (power
     *    configurations 5b/5s; operating condition VDD >= VDDUMIN = 4.35V).
     *    On 3.3V systems VUSB is fed externally (configuration 3s) and the
     *    datasheet directs USBVREG = 0. Selected by the Tools ->
     *    "VUSB Power Source" menu, which defines USB_VREG_INTERNAL. The
     *    explicit 0 write keeps a known state for bring-up/verification even
     *    if a bootloader left the regulator enabled. */
